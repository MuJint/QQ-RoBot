...
namespace Qiushui.Lian.Bot.Business
{
    ...
    internal class DealInstruction
    {
        ...
        {
            var uLogs = (await signLogsServices.Query(t => t.CmdType == CmdType.SpecialBonusPoints || t.CmdType == CmdType.SpecialPointsDeducted))?.OrderByDescending(t => t.CreateTime);
            if (uLogs.Count() <= 0 || uLogs == null)
                return;
            var strSb = new StringBuilder();
            strSb.Append($"时间      事例\r\n");
        ...
        public async ValueTask Morning(GroupMessageEventArgs eventArgs)
        {
            if (DateTime.Now.Hour > 7 && DateTime.Now.Hour < 9)
            {
                ...
                else
                {
                    ...
                    {
                        await SendMessageGroup(eventArgs, $"{_config.ConfigModel.NickName}把机会留给其他人吧~", true);
                        return;
                    }
                    else
                    {
            ...
            if (DateTime.Now.Hour > 19 && DateTime.Now.Hour < 24)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        await SendMessageGroup(eventArgs, $"{_config.ConfigModel.NickName}是第{nightLogs.Count}个早睡的人噢~美梦美梦，奖励2分", true);
                    }
                }
            }
        }
